...
class StackHolderImplTest {
    ...
    @Test
    fun WHEN_recreated_with_another_active_component_THEN_previous_retained_instance_destroyed() {
        val instanceKeeperDispatcher = InstanceKeeperDispatcher()
        val oldStackSaver = stackSaver()
        val oldHolder = stackHolder(stackSaver = oldStackSaver, instanceKeeperDispatcher = instanceKeeperDispatcher)
        val instance = TestInstance()
        oldHolder.stack.active.instanceKeeperDispatcher.put("key", instance)
        val savedState = oldStackSaver.save()

        val newStackSaver = stackSaver(isActiveEntryPreserved = false, savedState = savedState)
        stackHolder(stackSaver = newStackSaver, instanceKeeperDispatcher = instanceKeeperDispatcher)
    ...
    private fun stackSaver(
        isActiveEntryPreserved: Boolean = true,
        savedState: TestStackSaver.SavedState<Config>? = null
    ): TestStackSaver<Config> =
        TestStackSaver(
